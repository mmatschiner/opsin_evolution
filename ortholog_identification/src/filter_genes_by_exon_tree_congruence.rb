...
gene_alignment_directories.each do |dir|
    ...
    unless Dir.entries("#{alignment_directory_in}/#{dir}").include?("results.ccp")
        ...
        system("cp ../bin/concaterpillar/*.py* #{alignment_directory_in}/#{dir}")
        system("cp ../bin/raxmlHPC #{alignment_directory_in}/#{dir}")
...
# Edit the result file filtered_nuclear_exons.txt, removing exons that were filtered due to incongruence.
info_file_name = "../res/tables/filtered_nuclear_exons.txt"
